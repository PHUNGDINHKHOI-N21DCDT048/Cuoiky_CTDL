...
class Polynomial:
    ...
    def Tich(self, dathuc2):
        result = Polynomial()  # Tạo một đa thức mới để lưu kết quả

        # Duyệt qua từng số hạng của dathuc1
        current1 = self.head
        while current1 is not None:
            # Duyệt qua từng số hạng của dathuc2
            current2 = dathuc2.head
            while current2 is not None:
                heso = current1.heso * current2.heso
                somu = current1.somu + current2.somu
                result.Them(heso, somu)  # Thêm số hạng vào đa thức kết quả
